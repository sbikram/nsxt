...
func handleReadError(resourceType string, resourceID string, err error) error {
...
func handleDataSourceReadError(resourceType string, resourceID string, err error) error {
	msg := fmt.Sprintf("Failed to read %s %s", resourceType, resourceID)
	return logAPIError(msg, err)
}
